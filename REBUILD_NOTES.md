...
- **月度冠军 👑**：仅当某人「显著领先」时才产生，否则该月无人戴冠。三条须同时成立：① 唯一第一（并列无人戴冠）② 榜首 ≥ 2 次 ③ 榜首 ≥ 2×次高 或 领先次高 ≥ 2 次。判定实现见 `scripts/sitegen/reqstats.py` 的 `month_champions()`
- **跨月冠军**（v3.7.18 简化后只有三条规则）：① 连续两个自然月中点歌次数均为**唯一第一名**；② 只统计已完全结束的自然月，当前未结束的月份不计；③ 某月没有唯一第一名（并列 / 全员持平）则该月作废，断开与相邻月份的跨月配对，从下一个有效月份重新开始。判定实现见 `reqstats.compute_champion_streaks()`（每月唯一第一由 `month_leader()` 给出，并列即返回 None）。已移除原先的「并列亦计」「榜首须 ≥ 2 次」「显著领先」等额外门槛。当前结果 2 条：Vaserkia 2025-01—03（3 个月）、无响应失败小熊 2024-10—11（2 个月）
