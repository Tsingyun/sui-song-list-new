...
收录 **1,161** 首歌曲、**3,338** 次演唱记录，支持在线播放 B站录播片段。
...
+ 单文件 HTML 架构
  ...
  - 文件体积约 726 KB（含 1161 首歌曲完整数据 + 背景插画 WebP）
...
在线播放:
+ Bilibili iframe 播放器（复古电视机浮动小窗）
  - 点击播放按钮直接嵌入 B站高清播放器
  - 多版本歌曲支持切换不同录播片段（下拉选择器）
  - 左下角可拖拽浮动 16:9 小窗，套复古 CRT 电视机外框（兔耳天线 / 扫描线 / SUI◈TV 商标 / 电源灯 / 双旋钮）
  - 支持放大 / 还原，解决小窗下 B站 进度条与音量过小
  - 支持跳转 B站原视频链接
...
> 站点界面版本随功能迭代更新；详细记录同时显示在网页底部"更新日志"。

- **界面版本 v2.3** · 2026-07-21
  - 修复更新日志（页脚）展开后页面不自动滚动：展开时按内容高度把内容滚入视口（短内容贴底 / 长内容从标题起），桌面与移动端一致
- **界面版本 v2.2** · 2026-07-21
  - 移动端体验精修：修单击歌名误触复制（改详情内显式按钮）、搜索框 16px 防 iOS 缩放、刘海/Home 条安全区适配、横屏播放器可关闭、播放按钮放大到 42px、横屏手机布局、点按反馈、字号下限、弹窗锁背景滚动、播放时隐藏盲盒、拖拽用 visualViewport 防抖动、分类标签滚动吸附、搜索聚焦防键盘遮挡
- **界面版本 v2.1** · 2026-07-21
  - 移动端彻底重做：多断点（768/640/360）适配，歌单改卡片式并保留播放按钮，控件全宽、浮动按钮触控栈、图表与弹窗自适应
  - 播放器套上复古 CRT 电视机外框（兔耳天线 / 扫描线 / SUI◈TV 商标 / 电源灯 / 双旋钮装饰）
  - 放大/还原图标改为清晰的窗口展开 / 还原样式，放大改为左下角原地生长
  - 播放器新增放大/还原按钮，解决小窗下 B站 进度条与音量过小
  - 播放器重构：全宽底栏 → 左下角可拖拽浮动 16:9 小窗
  - 四个浮动按钮显示时机统一并加平滑渐变动画
  - 鼠标光标点击热区从头顶修正到小鸟翅膀
- **2026-07-07** — 中文译名批量补全（280 首外语歌中 82 首新增公认中文译名）
- **2026-06-23** — 移动端 / 平板响应式优化（2 行堆叠卡片、标签换行、原唱与日期完整显示）
- **2026-06-14** — 岁己SUI 角色插画背景层 + 毛玻璃增强 + 4 种位置切换（含横屏面部居中）
- **2026-06-14** — 自定义小鸟鼠标光标、网站 favicon（岁图标）、全量动画与字体排版优化
- **2026-06-14** — 语言分类锚点跳转白屏修复；批量合并 30 对重复歌曲；艺术家与歌名全面审计修复
- **早期基线（早于 2026-06-14）** — 四视角浏览 / 搜索高亮 / 多维排序筛选 / 23 种流派标签 / 歌曲详情面板 / 数据洞察图表（热力图·趋势·分布·原唱 Top20）/ 歌单导出 / 盲盒抽歌 / 在线播放 / 演唱日期悬浮提示 / 单击复制 / "我要补充" 共创 / 统计卡片筛选 / "点歌统计" 跳转
...
- [x] 移动端彻底重做（多断点 768/640/360 适配，歌单卡片式 + 保留播放按钮，控件全宽、浮动按钮触控栈、图表与弹窗自适应）
