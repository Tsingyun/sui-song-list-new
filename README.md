...
收录 **1,166** 首歌曲、**3,377** 次演唱记录，支持在线播放 B站录播片段。
...
+ 单文件 HTML 架构
  ...
  - 文件体积约 736 KB（含 1166 首歌曲完整数据 + 背景插画 WebP）
...
+ 企划/系列标签
  - 为有明确世界观/系列的歌曲企划打标（阳炎Project / ヨルシカ / EGOIST / supercell / ずっと真夜中でいいのに。/ 結束バンド / Mili）
  - 与流派标签共用筛选栏，可一键聚合某企划全部曲目
...
+ 悬浮日期列查看完整演唱历史
  ...
  - 最早日期橙色高亮，最近日期青色高亮
  - 数据覆盖 1,169 首歌曲（来自本地 Excel + suijisui.space）
...
- **联网核实**：加歌若对艺术家或语言不确定，必须先联网搜索确认，不得仅凭歌名字符或记忆推断（如 `JANE DOE` 歌名全英文，实为米津玄師×宇多田ヒカル 的日语合作曲）
- **原唱/翻唱甄别**：翻唱版本可能比原唱更火，联网时需甄别原唱与翻唱；`artist` 字段标注**原唱**（如「歩いても歩いても、夜空は僕を追いかけてくる」原唱为 n.k/IA，Sana 是翻唱版本，artist 维持 n.k/IA）
...
- 1168 首歌曲的完整数据（JSON 内嵌）
- 932 首歌曲的 B站视频匹配数据
- 1232 首歌曲的演唱日期记录（SONG_DATES 查找表）
...
> 本项目（含全部源码、数据及任何资源）完全基于 [MIT License](LICENSE) 开源，可自由用于任何用途（含商业用途），无需额外授权。
