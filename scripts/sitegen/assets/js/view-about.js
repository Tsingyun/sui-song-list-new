...
(function () {
  ...
  var CHANGELOG = [
    ['v3.7.15', '首页新增「久别重逢」板块（最近演出下方）：把隔了 2 年以上没唱、最近又重新登台的曲目单独列出，每首给出上次演唱日 →（断档 X 年 Y 个月）→ 复唱日的时间轴与累计次数，点歌名直达详情。判定在构建期完成（按自然年算断档、以档案最后一次演唱日为「最近」的锚点，从后往前找最近一次复唱，避免漏掉复唱后又唱过的歌），数据里没有复唱时板块整段不渲染；「最近演出」相应由 12 天收到 8 天，让新板块的底边与右栏「盲盒抽歌」对齐'],
  ...
  function render(params, container) {
    container.innerHTML =
      ...
      '<h2 class="sec-title">关于本站 <span class="sec-sub">SUI SONG ARCHIVE v3.7.15</span></h2></div>' +
