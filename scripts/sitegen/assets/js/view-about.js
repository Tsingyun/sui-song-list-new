...
(function () {
  ...
  var CHANGELOG = [
    ['v3.7.18', '简化「跨月冠军」的判定，只保留三条规则：① 连续两个自然月中点歌次数均为**唯一第一名**；② 只有已完全结束的自然月参与统计，当前未结束的月份不计；③ 某月没有唯一第一名（并列 / 全员持平）则该月作废，并断开它与相邻月份的跨月配对，从下一个有效月份重新开始。原先叠在上面的「并列亦计」「榜首须 ≥ 2 次」等额外门槛全部移除，月度榜的 👑（显著领先才戴冠）不受影响。名单相应调整为 2 位：Vaserkia 连 3 个月（2025-01—03）、无响应失败小熊 连 2 个月（2024-10—11）'],
    ['v3.7.17', '修复点歌页「跨月冠军」只剩 2 位的问题：v3.7.4 把本板块与月度榜的 👑 判定合并成了同一个「显著领先才戴冠」规则，而本板块的语义是「连续数月蝉联月度**榜首**」——两者不是一回事。2025-03 月度榜里 Vaserkia 以 7 次排第 1 却因只领先 1 次没有皇冠，跨月冠军就不认这一个月，连冠被硬生生截断。现两套判定解耦：👑 仍是「显著领先」的高亮（月度榜去噪，2026-09 全员各 1 次时不会满屏皇冠）；跨月冠军改回「月度榜首事实」，并列第一同样算登顶，唯一门槛是该月榜首至少 2 次（挡掉「全员各 1 次」凑出的伪连冠）。名单恢复为 4 位：Vaserkia 连 3 个月（2025-01—03）、无响应失败小熊 / 仙布着急唱灭歌 / L1f3n0w0v3R710w_ 各连 2 个月；靠并列月达成的会在行尾标注「与 N 人共享其中 N 个月的榜首」，与月度榜「该月无皇冠」不再矛盾'],
    ['v3.7.16', '观众详情里的歌曲列表改为按「最近点歌」倒序 —— 最近点的那首排最前，不再按累计次数排；同一天点多首时按次数多的在前，两者都相同再按歌名，顺序稳定可复现。本期段（X 月 / 季 / 年 点过的歌）与下方「过往歌曲」段同一口径'],
    ['v3.7.15', '首页新增「久别重逢」板块（最近演出下方）：把隔了 2 年以上没唱、最近又重新登台的曲目单独列出，每首给出上次演唱日 →（断档 X 年 Y 个月）→ 复唱日的时间轴与累计次数，点歌名直达详情。判定在构建期完成（按自然年算断档、以档案最后一次演唱日为「最近」的锚点，从后往前找最近一次复唱，避免漏掉复唱后又唱过的歌），数据里没有复唱时板块整段不渲染；「最近演出」相应由 12 天收到 6 天，让新板块的底边与右栏「盲盒抽歌」对齐'],
  ...
  function render(params, container) {
    container.innerHTML =
      ...
      '<h2 class="sec-title">关于本站 <span class="sec-sub">SUI SONG ARCHIVE v3.7.18</span></h2></div>' +
