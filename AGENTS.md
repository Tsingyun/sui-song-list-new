...
4. **观众等级**：1–5 级阈值 11/31/61/100；**月度冠军 👑**：仅当某人**显著领先**时才产生——① 唯一第一（并列则无人戴冢）② 榖首 ≥ 2 次（只被点 1 次不算）③ 显著领先：榖首 ≥ 2×次高 **或** 领先次高 ≥ 2 次。故「2 次 vs 其余各 1 次」是冢军，而「全员 1 次」「5 vs 4」「11 vs 11」都不是（该月一个皇冠都不显示）。判定唯一实现在 `reqstats.py` 的 `month_champions()`，月度榖与跨月冢军共用，前端（含最近 15 天）只读结果不自行重算
