...
- **数据规模**: 1165首歌曲、3374次演唱记录、932首歌有B站录播匹配（截至 2026-08-23）
...
├── data/
│   ├── song_data.json                  ← 核心数据库（1165首歌曲，含 stats、lang_counts 和 tags）
...
└── docs/
    ├── index.html                      ← 最终网站（~727KB，由 build_site.py 生成）
...
```json
{
  "stats": { "total": 1165, "performances": 3374, "frequent": 239, "occasional": 435, "once": 491 },
...
- **联网核实**: 加歌若对艺术家或语言不确定，必须先联网搜索确认（如 `JANE DOE` 歌名全英文，实为米津玄師×宇多田ヒカル 的日语合作曲）
- **原唱/翻唱甄别**: 联网搜索结果中翻唱版本可能比原唱更火，需甄别原唱与翻唱；`artist` 字段一律标注**原唱**而非翻唱者（如「歩いても歩いても、夜空は僕を追いかけてくる」原唱为 n.k/IA，Sana 仅为翻唱版本，artist 维持 n.k/IA）
- **原唱排行**: 按原唱歌手聚合
...
- **在线播放**: 左下角可拖拽浮动 16:9 小窗，套复古 CRT 电视机外框（兔耳天线 / 扫描线 / SUI◈TV 商标 / 电源灯 / 双旋钮）；支持放大/还原与多版本切换；多版本歌曲支持下拉切换
- **录播标题**: 点击可跳转B站视频页面
