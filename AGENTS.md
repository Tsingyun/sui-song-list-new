...
- **架构**: 单文件 HTML（约534KB），所有 CSS、JS、数据全部内嵌在 `docs/index.html` 中
- **视觉风格**: Vaporwave / 赛博朋克（Orbitron + Share Tech Mono 字体、霓虹光效、CRT 扫描线、透视网格）
- **数据规模**: 1218首歌曲、3320次演唱记录、941首歌有B站录播匹配（截至 2026-06-14）
...
├── data/
│   ├── song_data.json                  ← 核心数据库（1218首歌曲，含 stats 和 lang_counts）
│   ├── song_bilibili_map.json          ← 歌曲-视频匹配结果（941首歌）
...
└── docs/
    └── index.html                      ← 最终网站（约534KB，由 build_site.py 生成）
```
...
```json
{
  "stats": { "total": 1218, "performances": 3320, "frequent": 230, "occasional": 451, "once": 537 },
  "lang_counts": [ { "lang": "中文", "count": 706 }, { "lang": "日语", "count": 401 }, ... ],
