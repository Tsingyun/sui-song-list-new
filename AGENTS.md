...
4. **观众等级**：1–5 级阈值 11/31/61/100；**月度冠军 👑**：仅当某人**显著领先**时才产生——① 唯一第一（并列则无人戴冠）② 榜首 ≥ 2 次（只被点 1 次不算）③ 显著领先：榜首 ≥ 2×次高 **或** 领先次高 ≥ 2 次。故「2 次 vs 其余各 1 次」是冠军，而「全员 1 次」「5 vs 4」「11 vs 11」都不是（该月一个皇冠都不显示）。判定唯一实现在 `reqstats.py` 的 `month_champions()`，月度榜与「最近 15 天」共用，前端只读结果不自行重算。**跨月冠军不使用本判定**——它用 `month_leader()`（该月唯一第一），见下方第 24 条
...
15. **点歌冠军 👑 显著性规则（v3.7.4）**：冠冕只在某人**显著领先**时出现，杜绝「全员各 1 次全戴冠」的冗余。判定唯一实现 `reqstats.month_champions()`（月度榜与「最近 15 天」共用，前端只读结果不自行重算）。回归脚本 `.zcode/workspace/default/_verify_champion.py`：覆盖 4 个代表性月份（2026-09 全员各 1 次→0 冠、2024-10 6 vs 4→1 冠、2025-03 7 vs 6→0 冠、2025-07 4 vs 2→1 冠）+ 点歌之王卡恒为 1 冠 + 最近 15 天随当月冠军 + 跨月冠军 2 条，全部断言。
...
23. **观众点歌趋势图（`view-requests.js` 的 `audTrendHtml`，v3.7.14 立）**：
    数据从 `R.raw`（`{d,s,a}`）**前端即时派生**，不加进 payload（88 位观众 × 全量记录
    已在页面上，没必要再寄一份）。四条约束：
    ① **粒度自适应**：跨度 ≤14 个月按月、更长按季（`TREND_MONTH_MAX`）。28 个月的月度柱在
    320px 弹层里每根不足 10px，标签必然叠在一起；改按季后柱宽始终 ≥20px。改阈值前先量柱宽。
    ② **必须补齐零值时段**成空柱（2px 浅色痕）。补出的是「这条时间轴上的沉寂」，与「无数据」
    不是一回事，轴断了就失去时间语义。
    ③ **轴标签用最小索引间距贪心占位**，首/末/峰值优先，冲突时挤掉参照点。均匀取点 + 事后
    强制补峰值会让两个标签贴在一起（390px 实测过），不要退回那种写法。
    ④ **柱高比例 = 该时段次数 / 峰值**（不是 / 总数），峰值柱常显数值、其余悬停显示；
...
24. **跨月冠军 ≠ 月度皇冠，且只有三条规则（v3.7.18 简化）**：语义 =「连续两个自然月中点歌次数均为该月**唯一第一名**」，判定 `reqstats.compute_champion_streaks()`（每月唯一第一由 `month_leader()` 给出，并列即返回 None）。三条：① 连续两个自然月都是唯一第一（连续月合并为一段，2025-01—03 算一条 3 个月的）；② **只统计已完全结束的自然月**，当前未结束的月份整月排除；③ 某月没有唯一第一名（并列 / 全员持平）则该月**作废**，既无榜首，也**断开与相邻月份的跨月配对**，从下一个有效月份重新开始。月份一律取自数据，**禁止硬编码**任何具体月份；也不要再加「最低次数」「显著领先」「并列亦计」之类额外门槛 —— v3.7.17 正是因为叠了这些，条数才在 4 / 2 之间来回改。它与月度榜 👑（规则 15，`month_champions()`，要求显著领先）仍是**两套判定，禁止合并**。回归见 `.zcode/workspace/default/_verify_champion.py`（断言 2 条 + 未结束月不出现 + 并列月作废后 L1f3 不出现）
...
25. **观众详情歌曲列表的排序口径（v3.7.16 立）**：`view-requests.js` 的 `openAudience()` 里，
    「点过的歌 / 过往歌曲」列表**默认按「这首歌最近一次被该观众点」的日期倒序**（最近点的排最前），
    不再按累计次数排。五条约束：
    ① **排序键是 `songLast[歌]`（该曲最后一次点歌日），而且它就是行内显示的日期** ——
    两者必须同源，否则会出现「排序看着乱」的假象（历史月份下该曲可能在更晚的月份也点过，
    行内日期会晚于本期标题，这是既有显示行为，不是排序错）。
    ② 日期是 `YYYY-MM-DD`，**字典序即时间序**，直接用字符串比，不要 new Date() 解析。
    ③ tiebreak 顺序固定为「日期↓ → 次数↓ → 歌名↑（UTF-16 码元序，与 JS `<` 一致）」；
    实证脚本用 Python 重算时必须按 `s.encode('utf-16-be')` 比歌名 —— emoji 等 BMP 外字符
    的 code point 序与 JS 的 UTF-16 码元序不同（如 `🥕` 会排错位）。
    ④ **本期段与「过往歌曲」段必须用同一个比较函数**（`byRecent`）—— 曾经本期段是升序、
    主列表是降序，同一次展开里两段方向相反。
    ⑤ 两段并存时，最近点的那首**在本期段里**（因为它是本期点的），所以「最近在最前」这条断言
    要在「按 DOM 顺序拼接两段」的序列上判，只看最后一段必然失败。
    无排序开关、URL 不带排序参数、也不落任何存储状态（默认口径即唯一口径）。
    回归：`.zcode/workspace/default/_verify_aud_sort.py`（75 项：Python 独立重算全序列逐行比对、
    首行日期 == 该观众 max(date)、段内单调性、分段归属、月/季/年/总榜与历史月入口一致、
    幂等、URL 无排序状态、邻近弹层回归、三视口无横向溢出）。
